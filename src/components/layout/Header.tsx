...
const openWhatsApp = () => {
  const phoneNumber = '+254736315685';
...
export const Header = () => {
  ...
  return (
    ...
    >
      ...
      <div className="hidden lg:block bg-gradient-to-r from-amber-600 to-rose-600 text-white py-1.5">
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center space-x-6">
            <a href="tel:+254736315685" className="flex items-center text-sm hover:text-white transition-colors bg-red-600 px-3 py-1.5 rounded-full">
              <Phone className="w-4 h-4 mr-2" />
              +254736315685
            </a>
          </div>
          <div className="flex items-center space-x-4">
            <a href="#" className="text-sm hover:text-white/80 transition-colors">Find a Branch</a>
            <a href="#" className="text-sm hover:text-white/80 transition-colors">FAQ</a>
          </div>
        </div>
      </div>
      ...
      <nav className="container mx-auto px-4">
        <div className="flex items-center justify-between">
            ...
            {navItems.map((item) => (
              item.action ? (
                <button
                  key={item.name}
                  onClick={item.action}
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-all relative group text-white`}
                >
                  {item.name}
                  <motion.div
                    className="absolute inset-0 bg-gradient-to-r from-amber-600 to-rose-600 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                ...
                >
                  {item.name}
                  <motion.div
                    className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-amber-600 to-rose-600 scale-x-0 group-hover:scale-x-100 transition-transform origin-left ${
          ...
          <a
            href="tel:+254736315685"
            className="md:hidden flex items-center mr-4 bg-red-600 text-white rounded-full shadow-md hover:bg-red-700 transition-colors px-4 py-2 space-x-2"
            aria-label="Call Us"
          >
            <motion.div
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs font-medium">+254736315685</span>
            </div>
          </a>
