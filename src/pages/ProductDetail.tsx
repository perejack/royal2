...
export const ProductDetail = () => {
  ...
  // Calculate initial selected gauge if not set
  useEffect(() => {
    if (product.priceVariants && product.priceVariants.length > 0 && !selectedGauge) {
      // Extract gauge from the first price variant
      const firstVariant = product.priceVariants[0].name;
      const gaugeMatch = firstVariant.match(/(\d+)/);
      if (gaugeMatch) {
        setSelectedGauge(gaugeMatch[0]);
      }
    }
  }, [product, selectedGauge]);
  ...
  // Calculate total based on selected gauge
  const getTotal = useCallback(() => {
    if (!product.priceVariants || product.priceVariants.length === 0) return 0;
    
    const selectedVariant = product.priceVariants.find(v => {
      const gaugeMatch = v.name.match(/(\d+)/);
      return gaugeMatch && gaugeMatch[0] === selectedGauge;
    });
    
    if (!selectedVariant) return 0;
    
    // Extract numeric value from price string (e.g., "KSh 650 / meter" -> 650)
    const priceMatch = selectedVariant.price.match(/\d+/);
    const price = priceMatch ? parseFloat(priceMatch[0]) : 0;
    return price * quantity;
  }, [product.priceVariants, selectedGauge, quantity]);
  ...
  const handleSubmit = () => {
    ...
    window.open(`https://wa.me/+254736315685?text=${message}`, '_blank');
  };
